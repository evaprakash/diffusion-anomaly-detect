    ...
    def training_losses(self,
        ...
        if noise is None:
            if (enc_dims == 2):
                ...
                noise = th.randn_like(x_enc)
        
        if (enc_dims == 2):
            x_t = self.q_sample(x_start, t, noise=noise)
        else:
            x_t = self.q_sample(x_enc, t, noise=noise)
        ...
        if self.loss_type in [
            ...
            assert model_output.shape == target.shape == x_enc.shape
            ...
            if self.loss_type == LossType.mse:
                if self.model_mean_type == ModelMeanType.eps:
                    ...
                    if (kl_model is not None):
                        kl_div = kl_model.forward(model_output, target) 
                        torch.save(kl_model, "kl_model_200.pt")  
                else:
