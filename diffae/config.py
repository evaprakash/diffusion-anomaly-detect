...
data_paths = {
    ...
    'shanghailmdb':
    os.path.expanduser('datasets/ubnormal'),
    'video':
    os.path.expanduser('/vision2/u/eprakash/shanghai_anomaly/train_img_to_video.txt'),
    'celeba_anno':
    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return CelebAlmdb(path=path or self.data_path,
                              ...
                              crop_d2c=True,
                              **kwargs)
        elif self.data_name == 'shanghailmdb':
            return Shanghai_lmdb(path=path or self.data_path,
                              image_size=self.img_size,
                              original_resolution=None,
                              **kwargs)
        elif self.data_name == 'video':
            return Video_Dataset(path=path or self.data_path,
                              image_size=self.img_size,
                              original_resolution=None,
                              **kwargs)
        else:
