...
class LitModel(pl.LightningModule):
    ...
    def sample_load_cond(self, N, device, T=None, T_latent=None, conds_mean=None, conds_std=None):
        if T is None:
            sampler = self.eval_sampler
            latent_sampler = self.latent_sampler
        else:
    ...
    def train_dataloader(self):
        ...
        if self.conf.train_mode.require_dataset_infer():
            if self.conds is None:
                ...
                # (1, c)
                self.conds_mean = torch.empty((1, self.conds.shape[1]), dtype=torch.float)
                self.conds_std = torch.empty((1, self.conds.shape[1]), dtype=torch.float)
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                x_enc = None
                if (self.conf.enc_dims == 3):
                    imgs, idxs, frame_batches = batch['img'], batch['index'], batch['frame_batch']
                    x_enc = frame_batches
                else:
    ...
    def test_step(self, batch, *args, **kwargs):
        ...
        for each in self.conf.eval_programs:
            if each.startswith('fid'):
                ...
                conf = self.conf.clone()
                conf.eval_num_images = 5_000#50_000
                score = evaluate_fid(
                    sampler,
                    self.ema_model,
                    conf,
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    if mode == 'train':
        trainer.fit(model)
        #conds_save = model.conds.clone().detach().cpu().numpy()
        #with open('conds.txt') as f:
        #    np.save(f, conds_save)
    elif mode == 'eval':
